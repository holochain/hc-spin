...
if (!electronBinary) {
  ...
  if (electronHandleTemp.stdout) {
    if (!electronHandleTemp.stdout.toString().startsWith('v###REPLACE_AT_BUILD_TIME###')) {
      console.warn(
        'WARNING: Found a globally installed electron version but it does not match the version requirements of hc-spin (v###REPLACE_AT_BUILD_TIME###x). The electron binary from node_modules will be used instead.',
      );
    } else {
